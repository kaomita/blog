...
```
这里的ip和网关要根据路由器来设置，如果你的路由器提供的ipv4是`10.162.6.x`，那么你的ip应该设置为`10.162.6.x`（将x换为初1外你想要的任何数字），网关就是`10.162.6.1`
...
## shell
ubuntu server的shell光秃秃的，而且tab补全不能选择列出的文件。这里推荐安装zsh，然后安装oh-my-zsh，来增强shell的功能。
